...
public record WodMovementRequest(
    @Schema(
        ...
        String movementId,
    @Schema(description = "Sequence order in the WOD (1-based)", example = "1")
        @NotNull(message = "{validation.order.required}")
        @Min(value = 1, message = "{validation.order.positive}")
        Integer orderIndex,
    @Schema(description = "Rep scheme string (e.g., '21-15-9', '5x5')", example = "21-15-9")
        @Size(max = 50, message = "{validation.wod.repsScheme.size}")
        String repsScheme,

    // --- Prescription Data ---

    @Schema(description = "Prescribed weight value (if applicable)", example = "60.0")
        @DecimalMin(value = "0.0", message = "{validation.value.positive}")
        Double weight,
    @Schema(description = "Unit for the prescribed weight", example = "KG") Unit weightUnit,
    @Schema(description = "Target duration in seconds (for static holds or cardio)", example = "60")
        @Min(value = 0, message = "{validation.value.positive}")
        Integer durationSeconds,
    @Schema(description = "Preferred unit for displaying duration", example = "SECONDS")
        Unit durationDisplayUnit,
    @Schema(description = "Target distance value (if applicable)", example = "400.0")
        @DecimalMin(value = "0.0", message = "{validation.value.positive}")
        Double distance,
    @Schema(description = "Unit for the prescribed distance", example = "METERS") Unit distanceUnit,
    @Schema(description = "Target energy expenditure in calories", example = "20")
        @Min(value = 0, message = "{validation.value.positive}")
        Integer calories,

    // --- Instructions ---

    @Schema(description = "Specific notes for this movement in this WOD", example = "Touch and go")
